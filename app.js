(function () {
  ...
  function inferByHistoryOrHeuristic(activityLabel) {
    ...
    if (_historyInferCacheGen === _eventsMutationGen && _historyInferCache.has(key)) {
      return _historyInferCache.get(key);
    }
    if (_historyInferCacheGen !== _eventsMutationGen) {
      _historyInferCache.clear();
      _historyInferCacheGen = _eventsMutationGen;
    }
    for (let i = state.events.length - 1; i >= 0; i--) {
      ...
      if (nm === key) {
        ...
        _historyInferCache.set(key, hit);
        return hit;
      }
    }
    ...
    if (workSet.has(key)) {
      const hit = { group: "Work", layer: "Freedom", cat: "Time" };
      _historyInferCache.set(key, hit);
      return hit;
    }
    const fallback = { group: "Rest", layer: "Health", cat: "Mental Health" };
    _historyInferCache.set(key, fallback);
    return fallback;
  }
  ...
  function showApprovalPanel(payload) {
    ...
    meta.innerHTML =
      ...
      `</div>`;
    list.innerHTML = "";
    const groupOpts = CLASSIFICATION_GROUP_OPTIONS.slice();
    const layerOpts = CLASSIFICATION_LAYER_OPTIONS.slice();
    const catOpts = CLASSIFICATION_CAT_OPTIONS.slice();
    const subOpts = CLASSIFICATION_SUB_OPTIONS.slice();
    ...
    payload.candidates.forEach((c) => {
      ...
      row.innerHTML =
        ...
        `<div>Group</div><div>${mkSel(`map-g-${editableId}`, groupOpts, c.group || groupOpts[0], false)}</div>` +
        `<div>Layers</div><div>${mkSel(`map-l-${editableId}`, layerOpts, c.layer || layerOpts[0], false)}</div>` +
        `<div>Cat</div><div>${mkSel(`map-c-${editableId}`, catOpts, normalizeCatDisplayForRaw(c.cat) || c.cat || catOpts[0], false)}</div>` +
        `<div>Sub Cat</div><div>${mkSel(`map-s-${editableId}`, subOpts, c.subCat || subOpts[0], false)}</div>` +
